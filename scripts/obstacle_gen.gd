...
func spawn_obstacle(obstacle_name, y = 300, x_displacement=0):
	...
	#obstacle.size = Vector2(500,randi_range(50, 200))
...
func _on_timer_timeout():
	...
	if randf() > .5:
		spawn_obstacle("static_wall", randi_range(30, GameParameters.WINDOW_HEIGHT-30))
	else:
		var separation = randi_range(GameParameters.PLAYER_SIZE + 100, 250)
		...
		spawn_obstacle("static_wall",y_first,x_delay)
		spawn_obstacle("static_wall",y_second)
